...
def _get_voc_results_file_template(json_dataset, salt):
    ...
    return os.path.join(devkit_path, 'results', domain, 'Main', filename)
...
def voc_info(json_dataset):
    domain = json_dataset.name.split('_')[-2]
    ...
    anno_path = os.path.join(
        devkit_path, domain, 'Annotations', '{:s}.xml')
    image_set_path = os.path.join(
        devkit_path, domain, 'ImageSets', 'Main', image_set + '.txt')
    return dict(
        domain=domain,
        image_set=image_set,
